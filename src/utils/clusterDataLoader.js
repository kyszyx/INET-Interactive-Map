 ...
 * @param {number} clusterId - Cluster ID (0-indexed, represents temperature rank)
 * @returns {Object} Cluster color info
 */
export const getClusterColor = (clusterId) => {
  // Cluster IDs are 0-indexed and represent temperature rank
  // ID 0 = hottest = Orange, ID 1 = 2nd hottest = Blue, etc.
...
export const getYearClusterData = async (year) => {
  ...
  try {
    ...
    // NOTE: cluster_data_clean.json is already sorted by temperature (hottest first)
    // Cluster IDs directly represent temperature rank: ID 0 = hottest, ID 1 = 2nd hottest, etc.
    // This matches the PNG tiles generated by recolor_dbscan_by_temperature.py

    // Assign colors based on cluster ID (which is the temperature rank)
    const clusterDataWithColors = yearClusterData.map((cluster) => ({
      ...cluster,
      color: CLUSTER_COLORS[cluster.id % CLUSTER_COLORS.length], // Use cluster ID for color
      temperatureRank: cluster.id // Cluster ID IS the temperature rank
    }));
